...
export default function CarMarketplace(){
  ...
  useEffect(()=>{const controller=new AbortController(),timer=setTimeout(()=>{setSearching(true);setPage(1);fetch(`/api/vehicles?zip=10001&limit=20&page=1&q=${encodeURIComponent(query)}`,{signal:controller.signal}).then(r=>r.json()).then(({data,mode,total,hasMore})=>{setDataMode(mode);setTotal(total);setHasMore(!!hasMore);setInventory(mode==="live"?normalize(Array.isArray(data)?data:[]):[])}).catch(e=>{if(e.name!=="AbortError"){setDataMode("error");setInventory([])}}).finally(()=>setSearching(false))},query?450:0);return()=>{clearTimeout(timer);controller.abort()}},[query]);
