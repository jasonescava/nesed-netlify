...
export default function CarMarketplace(){
  ...
  useEffect(()=>{const controller=new AbortController(),timer=setTimeout(()=>{setSearching(true);setPage(1);const pages=query?[1]:[1,2,3,4];Promise.all(pages.map(p=>fetch(`/api/vehicles?zip=10001&limit=20&page=${p}&q=${encodeURIComponent(query)}`,{signal:controller.signal}).then(r=>r.json()))).then(results=>{const first=results[0]||{};const merged=results.flatMap(x=>Array.isArray(x.data)?normalize(x.data):[]);setDataMode(first.mode);setTotal(first.total);setHasMore(results.some(x=>x.hasMore));setPage(pages[pages.length-1]);setInventory(first.mode==="live"?[...new Map(merged.map(x=>[x.id,x])).values()]:[])}).catch(e=>{if(e.name!=="AbortError"){setDataMode("error");setInventory([])}}).finally(()=>setSearching(false))},query?450:0);return()=>{clearTimeout(timer);controller.abort()}},[query]);
